...
> **v2.0** — 무거운 Electron(크롬 엔진 내장) 제거. 컨트롤 패널은 이제 **로컬 웹 서버 + 기존 브라우저** 방식이라 훨씬 가볍고 안정적입니다.
...
npm start
```
- `npm start` 하면 **기본 브라우저에 컨트롤 패널**(http://localhost:8787)이 자동으로 열립니다. (Electron/크롬 엔진 없음 — 가벼움)
- 패널에서 토큰/Application ID 입력 → **저장** → **▶️ 켜기**. (`.env`에 넣어두면 자동으로 채워짐)
- 켜지면 어느 서버에 있는지, 뭘 재생 중인지 실시간으로 보입니다.
- **이 터미널 창을 닫으면 봇도 꺼집니다.** 켜두는 동안만 음악이 나옵니다.
- 창 없이 터미널로만 돌리려면: `npm run cli`
